...
v0.3.2 是源码预览。可以运行测试和纯界面演示；不能据此直接在空白 Mac 上安装完整的 ChatGPT 本机服务。
...
`context/` 包含同步器及 macOS 授权助手源码，未包含个人配置、源记录、导出副本或已签名应用。复用时需要自行指定来源和输出目录，授权助手只能由用户通过系统界面授予目录权限。先读 [上下文说明](../context/README.md)。

`scripts/install-assistant.mjs --apply` 只更新已具备有效目录授权、上下文配置、原生助手及既有 LaunchAgent 的部署。`scripts/install-cli.mjs` 同样依赖既有本机基线；CLI 注册配置需要核对安装位置、身份、参数规则和保护范围。两者都不是空白电脑安装器。

`runtime/serve.py` 和 `runtime/control.py` 是去掉个人路径的启动器参考，默认使用用户主目录。Python、Node、业务 CLI 的位置和沙箱允许项仍需配套检查，不应直接覆盖任意现有启动脚本。公开示例工作台路径统一为 `Workspace/Workbench`。
